...
def parse_input(in_string):
    ...
    return int(lines[0]),\
        [ int(x) for x in lines[1].split(",") if digits.match(x.strip())  ],\
        [ int(x) if digits.match(x.strip()) else x.strip() for x in lines[1].split(",")  ]
